...
async def process_post_data(post_line: str, semaphore: asyncio.Semaphore) -> None:
    """Process a single post with semaphore-controlled concurrency and its own DB session."""
...
async def main() -> None:
    ...
    # Create tasks for all posts — each task creates its own DB session
    tasks = []
    for post_line in post_lines:
        if post_line:  # Skip empty lines
            task = asyncio.create_task(process_post_data(post_line, semaphore))
            tasks.append(task)
